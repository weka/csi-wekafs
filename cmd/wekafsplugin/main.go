...
func main() {
	...
	if enableMetrics != nil && *enableMetrics {
		...
		// Registered and set in every mode, and before any request is served, so that every plugin
...
func handle(ctx context.Context) {
	config := wekafs.NewDriverConfig(wekafs.DriverConfigOptions{
		...
		ReportQuotaMismatchAsAbnormal: *reportQuotaMismatchAsAbnormal,

		MutuallyExclusiveMountOptions: mutuallyExclusiveMountOptionsStrings,

		MaxCreateVolumeReqs:        *maxConcurrentCreateVolumeReqs,
		MaxDeleteVolumeReqs:        *maxConcurrentDeleteVolumeReqs,
		MaxExpandVolumeReqs:        *maxConcurrentExpandVolumeReqs,
		MaxCreateSnapshotReqs:      *maxConcurrentCreateSnapshotReqs,
		MaxDeleteSnapshotReqs:      *maxConcurrentDeleteSnapshotReqs,
