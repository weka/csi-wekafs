...
// Re-adding the same object at a new location has to clear the old filesystem/inode mapping.
//
// Add takes the caller's *VolumeMetric, and a caller updating a volume in place passes back the one
// already in the map. Writing the new key onto it before unindexing made unindexLocked search the
// volume's NEW location, find nothing there, and leave the old mapping behind - unreachable by
// Remove, which unindexes by the same key, and still answered by ForInode.
func TestVolumeMetricsReAddOfTheSameObjectClearsTheOldInode(t *testing.T) {
	vms := NewVolumeMetrics()
	fsOld, fsNew := uuid.New(), uuid.New()
	const inodeOld, inodeNew = uint64(11), uint64(22)
	pv := types.UID("pv-1")

	vm := &VolumeMetric{}
	vms.Add(pv, volumeKey{filesystemUid: fsOld, inodeId: inodeOld}, vm)
	if got := vms.ForInode(fsOld, inodeOld); len(got) != 1 {
		t.Fatalf("precondition: expected the volume at its first location, got %d", len(got))
	}

	// The same pointer, moved. This is the case that breaks.
	vms.Add(pv, volumeKey{filesystemUid: fsNew, inodeId: inodeNew}, vm)

	if got := vms.ForInode(fsOld, inodeOld); len(got) != 0 {
		t.Errorf("expected the old filesystem/inode mapping to be gone, still returns %d", len(got))
	}
	if got := vms.ForInode(fsNew, inodeNew); len(got) != 1 {
		t.Errorf("expected the volume at its new location, got %d", len(got))
	}

	// And Remove must leave nothing behind, which it cannot do if the stale mapping survived.
	vms.Remove(pv)
	if got := vms.ForInode(fsOld, inodeOld); len(got) != 0 {
		t.Errorf("expected no trace at the old location after Remove, got %d", len(got))
	}
	if got := vms.ForInode(fsNew, inodeNew); len(got) != 0 {
		t.Errorf("expected no trace at the new location after Remove, got %d", len(got))
	}
}
