...
func (s *Server) routes() *http.ServeMux {
	...
	// camelCase, like every other route here, because that is what the real API serves. This one
	// used to be registered lowercase to match how the client spelled it, which meant the fake
	// accommodated a client bug instead of exposing it - the listing 404s against a real cluster.
	// The client now builds it from FileSystem's own path helper, so both agree on one spelling.
