...
func (q *QuotaListRequest) getApiUrl(a *ApiClient) string {
	// Built from Quota's own path helpers rather than assembled here. The literal this replaced
	// spelled the collection "filesystems", where the Weka API and every other request in this
	// client use "fileSystems", so the list would have 404'd the moment anything called it. Going
	// through the helper means the two cannot disagree again.
	return (&Quota{FilesystemUid: q.FilesystemUid}).GetBasePath(a)
}
