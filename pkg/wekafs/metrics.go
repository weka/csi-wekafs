...
// csiDurationBuckets covers the range CSI operations actually occupy. The Prometheus defaults stop
// at 10s, while grpcRequestTimeout alone defaults to 30s, so every slow create, delete, publish and
// semaphore wait - the samples these histograms exist to expose - would otherwise land together in
// +Inf, and a 12s create would be indistinguishable from one that timed out. Matches the range the
// Weka API request histograms already use, so the two can be read on the same axis.
var csiDurationBuckets = []float64{0.1, 0.25, 0.5, 1, 2.5, 5, 7.5, 10, 15, 30, 60, 120, 300}

// buckets is optional: omitted, the histogram gets csiDurationBuckets, which is what the
// request-duration histograms every other caller of this function builds want. A caller measuring
// something on a different scale - the volume health sweep - passes its own.
func newHistogramVec(subsystem, name, help string, labels []string, buckets ...float64) *prometheus.HistogramVec {
	if len(buckets) == 0 {
		buckets = csiDurationBuckets
	}
...
// pluginInfo is exported by every plugin process from the moment it starts, before it has served a
// single RPC.
//
// Every other metric here is a CounterVec or GaugeVec, and those export nothing at all until a
// label set is first observed - so on a fresh install, or after a rollout or a leader failover on a
// quiet cluster, there is no series naming the process. Anything asking "which pods and namespaces
// is this driver running in" - dashboard variables above all - then gets an empty answer and
// silently queries nothing, which renders as a healthy zero rather than as no data.
//
// The mode label states the role once, as a label. The CSI operation metrics already imply it in
// their names - the subsystem is "controller" or "node", so weka_csi_node_publish_volume_total comes
// only from a node process, the leading "node" there being the subsystem and the one in
// "node_publish_volume" being part of the RPC name. But that only holds for metrics that exist per
// role: the Weka API request metrics are the same series from either, so nothing in them says which
// process made the call. A label answers uniformly, for both.
var pluginInfo = prometheus.NewGaugeVec(prometheus.GaugeOpts{
	Namespace: MetricsPrefix, Name: "plugin_info",
	Help: "Always 1, exported once per plugin process so that the process is discoverable before it serves any request",
}, slices.Concat(CsiCommonLabels, []string{"mode", "version"}))

// SetPluginInfo publishes the one always-present series for this process. Called once at startup.
func SetPluginInfo(driverName string, mode CsiPluginMode, version string) {
	pluginInfo.WithLabelValues(driverName, string(mode), version).Set(1)
}

// PluginInfoCollector is registered in every mode, unlike the controller and node collectors.
func PluginInfoCollector() prometheus.Collector { return pluginInfo }
