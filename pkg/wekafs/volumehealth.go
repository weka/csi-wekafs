...
func (cs *ControllerServer) describeVolume(ctx context.Context, pv *v1.PersistentVolume, filesystems *filesystemCache) (volume *csi.Volume, condition *csi.VolumeCondition, labels []string, err error) {
	...
	if err != nil {
		if errors.Is(err, ErrVolumeHealthUndetermined) {
			...
			return volume, nil, labels, nil
		}
		return volume, nil, labels, status.Errorf(codes.Internal, "failed to determine condition of volume %s: %v", volumeID, err)
	}
	...
	return volume, &csi.VolumeCondition{Abnormal: health.Abnormal, Message: health.Message}, labels, nil
}
