...
func (d *WekaFsDriver) ensureMountOptionsConfigMap(ctx context.Context) (*v2.ConfigMap, error) {
	logger := log.Ctx(ctx)
	client := d.GetK8sApiClient()
	if client == nil {
		log.Error().Msg("Failed to get Kubernetes client")
	...
	configMap, err := client.CoreV1().ConfigMaps(getOwnKubernetesNameSpace()).Get(ctx, getMountOptionsConfigMapName(d.name), v1.GetOptions{})
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
			...
			configMap, err = client.CoreV1().ConfigMaps(getOwnKubernetesNameSpace()).Create(ctx, configMap, v1.CreateOptions{})
		} else {
...
func (d *WekaFsDriver) SetVolumeMountOptionsInMap(ctx context.Context, volumeId string, options string) error {
	...
	// we assume that at this stage we already run in Kubernetes otherwise getOwnKubernetesNameSpace() will fail
	_, err = client.CoreV1().ConfigMaps(getOwnKubernetesNameSpace()).Update(ctx, c, v1.UpdateOptions{})
	if err != nil {
...
func (d *WekaFsDriver) DeleteVolumeMountOptionsFromMap(ctx context.Context, volumeId string) {
	...
	// we assume that at this stage we already run in Kubernetes otherwise getOwnKubernetesNameSpace() will fail
	_, err = d.GetK8sApiClient().CoreV1().ConfigMaps(getOwnKubernetesNameSpace()).Update(ctx, c, v1.UpdateOptions{})
	if err != nil {
		log.Error().Err(err).Msg("Failed to update config map")
		return
	}
