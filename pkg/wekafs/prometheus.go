...
var (
	...
	// LabelsForFilesystemOps identifies one filesystem, for the metrics server's own per-filesystem
	// work counters. A filesystem is not a volume, so these deliberately do not carry the volume set.
...
func NewPrometheusMetrics() *PrometheusMetrics {
	...
	m.server.FetchPvBatchOperationsInvokeCount = newServerCounter("fetch_pv_batch_operations_invoke_count_total", "Total number of operations to fetch PersistentVolume objects from Kubernetes API")
	...
	m.server.FetchPvBatchOperationsDurationSeconds = newServerCounter("fetch_pv_batch_operations_duration_seconds_total", "Total duration of operations to fetch PersistentVolume objects from Kubernetes API in seconds")

	m.server.FetchPvBatchOperationsDurationHistogram = newServerHistogram("fetch_pv_batch_operations_duration_seconds", "Histogram of durations for fetching PersistentVolume objects from Kubernetes API")
	...
	m.server.ProcessPvOperationsDurationSeconds = newServerCounter("process_pv_operations_duration_seconds_total", "Total duration of processing PersistentVolume objects in seconds")

	m.server.ProcessPvOperationsDurationHistogram = newServerHistogram("process_pv_operations_duration_seconds", "Histogram of durations for processing PersistentVolume objects")
	...
	m.server.FetchMetricsBatchOperationsDurationSeconds = newServerCounter("fetch_metrics_batch_operations_duration_seconds_total", "Total duration of fetch metrics batches from Weka cluster in seconds")

	m.server.FetchMetricsBatchOperationsDurationHistogram = newServerHistogram("fetch_metrics_batch_operations_duration_seconds", "Histogram of durations for fetching metrics batches from Weka cluster")
	...
	m.server.FetchSinglePvMetricsOperationsInvokeCount = newServerCounter("fetch_single_pv_metrics_operations_invoke_count_total", "Total number of single metrics fetch operations from Weka cluster")

	m.server.FetchSinglePvMetricsOperationsSuccessCount = newServerCounter("fetch_single_pv_metrics_operations_success_count_total", "Total number of single metrics fetch operations from Weka cluster that were completed successfully")

	m.server.FetchSinglePvMetricsOperationsFailureCount = newServerCounter("fetch_single_pv_metrics_operations_failure_count_total", "Total number of single metrics fetch operations from Weka cluster that failed")

	m.server.FetchSinglePvMetricsOperationsDurationSeconds = newServerCounter("fetch_single_pv_metrics_operations_duration_seconds_total", "Total duration of single metrics fetch operations from Weka cluster in seconds")

	m.server.FetchSinglePvMetricsOperationsDurationHistogram = newServerHistogram("fetch_single_pv_metrics_operations_duration_seconds", "Histogram of durations for fetching single metrics from Weka cluster")
	...
	m.server.PruneVolumesBatchDurationSeconds = newServerCounter("prune_volumes_batch_duration_seconds_total", "Total duration of prune volumes batch operations in seconds")

	m.server.PruneVolumesBatchDurationHistogram = newServerHistogram("prune_volumes_batch_duration_seconds", "Histogram of durations for prune volumes batch operations")
	...
	m.server.QuotaMapRefreshDurationSeconds = newFsCounterVec("quota_map_refresh_duration_seconds_total", "Total duration of quota map updates per filesystem in seconds")

	m.server.QuotaMapRefreshDurationHistogram = newFsHistogramVec("quota_map_refresh_duration_seconds", "Histogram of durations for quota map updates per filesystem")
	...
	m.server.QuotaUpdateBatchDurationSeconds = newServerCounter("quota_update_batch_duration_seconds_total", "Total duration of all quota update batches in seconds")

	m.server.QuotaUpdateBatchDurationHistogram = newServerHistogram("quota_update_batch_duration_seconds", "Histogram of durations for quota update batches")
