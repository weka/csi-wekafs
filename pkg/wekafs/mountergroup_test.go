...
// GetPreferredMounter returns nil once Probe has disabled every transport, and several callers used
// to call methods on that result unguarded - a panic taking the process down rather than one failed
// request. MountUnderlyingFS already guarded; Trash and NodeGetInfo did not.
func TestNoAvailableTransportIsReportedNotPanicked(t *testing.T) {
	ctx := context.Background()

	// Everything disabled: the state a node reaches when Probe finds no usable transport.
	nfs := &nfsMounter{}
	wekafs := &wekafsMounter{}
	nfs.Disable()
	wekafs.Disable()
	mg := &MounterGroup{nfs: nfs, wekafs: wekafs}
	if got := mg.GetPreferredMounter(ctx); got != nil {
		t.Fatalf("precondition: expected no preferred mounter with every transport disabled, got %v", got)
	}

	// Trash must report it rather than dereference nil. innerPath set and not on a snapshot, so
	// requiresGc is true and the nil mounter is actually reached.
	vol := &Volume{server: &ControllerServer{mounters: mg}, FilesystemName: "fs", innerPath: "x"}
	func() {
		defer func() {
			if r := recover(); r != nil {
				t.Errorf("expected Trash to report the missing transport, it panicked: %v", r)
			}
		}()
		if err := vol.Trash(ctx); err == nil {
			t.Error("expected Trash to fail when no transport is available")
		}
	}()

	// MountUnderlyingFS had the guard already, but called getMounter a second time to use the
	// result - so with Probe disabling transports concurrently the check and the use could disagree.
	// It now resolves once; this covers the guard itself.
	func() {
		defer func() {
			if r := recover(); r != nil {
				t.Errorf("expected MountUnderlyingFS to report the missing transport, it panicked: %v", r)
			}
		}()
		if err, _ := vol.MountUnderlyingFS(ctx); err == nil {
			t.Error("expected MountUnderlyingFS to fail when no transport is available")
		}
	}()
}

// The Kubernetes node label and the CSI topology segment must name the same transport. They were
// derived from different sources - the label from its own isWekaRunning call, the topology from the
// mounter Probe had just enabled - so a node could advertise wekafs topology to the scheduler while
// its wekafs mounter was disabled, and attract the workloads it had just stopped being able to mount.
func TestNodeTransportLabelFollowsTheEnabledMounters(t *testing.T) {
	ctx := context.Background()

	for name, tc := range map[string]struct {
		nfsEnabled, wekafsEnabled bool
		want                      string
	}{
		"wekafs only":                {false, true, string(dataTransportWekafs)},
		"nfs only, i.e. failed back": {true, false, string(dataTransportNfs)},
		"both, wekafs is preferred":  {true, true, string(dataTransportWekafs)},
		"none, nothing to advertise": {false, false, ""},
	} {
		nfs := &nfsMounter{}
		wekafs := &wekafsMounter{}
		if tc.nfsEnabled {
		...
		if got := nodeTransportLabel(ctx, mg); got != tc.want {
			t.Errorf("%s: nodeTransportLabel = %q, want %q", name, got, tc.want)
		}

		// And it is the same answer NodeGetInfo reports, which is the point of sharing the call.
		ns := &NodeServer{mounters: mg}
		viaNodeServer := ""
		if m := ns.getMounter(ctx); m != nil {
			viaNodeServer = string(m.getTransport())
		}
		if viaNodeServer != tc.want {
			t.Errorf("%s: NodeGetInfo would report %q while the node label says %q", name, viaNodeServer, tc.want)
		}
	}

	// A driver whose mounters were never built must not be reported as serving a transport.
	if got := nodeTransportLabel(ctx, nil); got != "" {
		t.Errorf("nodeTransportLabel with no mounter group = %q, want empty", got)
	}
}
