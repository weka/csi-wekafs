...
func (cs *ControllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	defer func() {
		...
		driverName := cs.getConfig().GetDriver().name
		// Guarded the way ExpandVolume already guards its own add. A Prometheus counter panics on a
		// negative Add, RequiredBytes comes straight from the caller and is not the driver's to
		// trust, and this runs in a defer - so the panic would replace the gRPC return and take the
		// controller down rather than failing one request.
		if required := req.GetCapacityRange().GetRequiredBytes(); required > 0 {
			controllerMetrics.Operations.CreateVolumeTotalCapacity.WithLabelValues(driverName, result, bt).Add(float64(required))
		}
