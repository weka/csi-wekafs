...
func (vms *VolumeMetrics) Add(pvUID types.UID, key volumeKey, vm *VolumeMetric) {
	...
	//
	// And the new key is written only after that, because the caller may be re-adding the very
	// object already in the map. Assigning vm.key up front would overwrite the key unindexLocked
	// looks the old entry up by, so it would search the volume's new location, find nothing, and
	// leave a byFsInode mapping at the old one that Remove can never reach and ForInode still
	// answers from.
