...
func (ns *NodeServer) NodeGetInfo(ctx context.Context, req *csi.NodeGetInfoRequest) (*csi.NodeGetInfoResponse, error) {
	...
	if ns.config.manageNodeTopologyLabels {
		...
		// Omitted rather than dereferenced when no transport is available: getMounter returns nil
		// once Probe has disabled them all, and reporting topology is not worth crashing the node
		// plugin over. A node with no usable transport should not be advertising one anyway.
		if mounter := ns.getMounter(ctx); mounter != nil {
			segments[fmt.Sprintf(TopologyLabelTransportPattern, driverName)] = string(mounter.getTransport())
		} else {
