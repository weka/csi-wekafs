...
func (ns *NodeServer) NodeGetVolumeStats(ctx context.Context, req *csi.NodeGetVolumeStatsRequest) (*csi.NodeGetVolumeStatsResponse, error) {
	...
	// Instrumented before validation rather than after it, as the other handlers are. The failures
	// below - an empty id or path, a path that is not a Weka mount - are the common kubelet cases,
	// and a handler that only counts the requests which got past validation cannot show them.
	start := time.Now()
	result := "FAILURE"
	defer func() {
	...
	// Check that the volume path is indeed a Weka mount (either NFS or WekaFS)
...
func (ns *NodeServer) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	defer func() {
		...
		driverName := ns.getConfig().GetDriver().name
		// The metric's status is the CSI outcome, which is what a success-rate query is asking
		// about. SUCCESS_WITH_WARNING returns success to the caller - the target path existed but
		// was not a Weka mount, so there was nothing to unmount - and counting it as a third status
		// would leave those unpublishes out of both the success and the failure series. The log
		// keeps the distinction, since that is where the warning is worth reading.
		metricResult := result
		if metricResult == "SUCCESS_WITH_WARNING" {
			metricResult = "SUCCESS"
		}
		recordOperation(nodeMetrics.Operations.UnpublishVolume, nodeMetrics.Operations.UnpublishVolumeDuration, start, driverName, metricResult)
		logger.WithLevel(level).Str("result", result).Msg("<<<< Completed processing request")
	}()
...
func (ns *NodeServer) NodeGetInfo(ctx context.Context, req *csi.NodeGetInfoRequest) (*csi.NodeGetInfoResponse, error) {
	...
	if ns.config.manageNodeTopologyLabels {
		...
		// Omitted rather than dereferenced when no transport is available: every mounter can be
		// disabled once Probe has been through them, and reporting topology is not worth crashing
		// the node plugin over. A node with no usable transport should not be advertising one.
		//
		// Same call the Kubernetes node label is set from, so the two always agree.
		if transport := nodeTransportLabel(ctx, ns.mounters); transport != "" {
			segments[fmt.Sprintf(TopologyLabelTransportPattern, driverName)] = transport
		} else {
			logger.Error().Msg("No transport available, omitting the transport topology label")
		}
